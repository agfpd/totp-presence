...
MCP-сервер — stdio-процесс. Добавляется записью в конфигурацию
клиента.
...
Python через pyenv/conda, убедиться что MCP-клиент запускает нужный
python — при необходимости указать полный путь в `"command"`.
