English version: [README.md](./README.md)

# mcp-server — MCP-интеграция для totp-presence

MCP-сервер (Model Context Protocol), который даёт агенту инструменты
для проверки идентичности владельца и управления сессией. В отличие
от hook, агент сам решает, когда вызвать проверку — заранее, до
действия, а не задним числом по отказу.

Работает в любом MCP-клиенте: Claude Code, Claude Desktop, Cursor,
Continue — всё, что умеет запускать MCP-сервер по стандартному
ввод-выводу.

## Три инструмента

### `totp_verify(code, integration)`

Верификация кода и открытие сессии. Агент получает код от владельца
и вызывает:

Запрос: `totp_verify("847291", "claude-code")`

Ответ при успехе:
```json
{
  "valid": true,
  "session_opened": true,
  "integration": "claude-code",
  "window_seconds": 1500,
  "opened_at": 1775914871,
  "expires_at": 1775916371
}
```

При неверном коде или блокировке — возвращает ошибку с описанием.
При блокировке (5 подряд неверных кодов, 5 минут ожидания) в тексте
ошибки будет время до разблокировки. Повторять не нужно.

### `totp_check_session(integration)`

Неинвазивная проверка — открыта ли сессия:

```json
// сессия открыта
{ "open": true, "expires_in_seconds": 1266 }

// сессия просрочена
{ "open": false, "reason": "session expired" }
```

Позволяет агенту узнать состояние до действия, не дожидаясь отказа
от hook.

### `totp_status()`

Диагностика — что установлено, какие интеграции видны:

```json
{
  "core_installed": true,
  "integrations": [
    { "integration": "claude-code", "open": true, ... }
  ]
}
```

## Зачем это поверх hook

Hook — жёсткий блок: агент узнаёт о проблеме только когда уже
споткнулся. MCP-инструменты добавляют упреждающий слой: агент
проверяет сессию заранее, принимает решения на основе
структурированного контекста (окно, возраст, время до истечения).

Особенно ценно, когда:
- Действие формально не защищено hook, но агент считает его важным.
- Клиент не поддерживает hooks (Claude Desktop, Cursor, Continue).

## Установка

### Требования

1. Ядро: `sudo ./core/setup.sh install`
2. Хотя бы одна установленная интеграция (например
   `sudo ./examples/claude-code-hook/install.sh` создаёт
   `/etc/totp-presence/claude-code-config` и регистрирует
   интеграцию; соответствующий файл сессии создаётся по требованию
   под `/var/run/totp-presence/<пользователь>/claude-code-session`
   при первом успешном TOTP-коде).
3. Python 3.9+ и FastMCP:
   ```sh
   pip3 install fastmcp
   ```

### Подключение к MCP-клиенту

MCP-сервер — процесс на стандартном вводе-выводе. Добавляется записью
в конфигурацию клиента.

**Claude Code** (`~/.claude.json`):
```json
{
  "mcpServers": {
    "totp-presence": {
      "command": "python3",
      "args": ["/absolute/path/to/examples/mcp-server/server.py"]
    }
  }
}
```

После перезапуска появятся инструменты:
`mcp__totp-presence__totp_verify`,
`mcp__totp-presence__totp_check_session`,
`mcp__totp-presence__totp_status`.

**Claude Desktop** (`~/Library/Application Support/Claude/claude_desktop_config.json` на macOS) — тот же формат `mcpServers`.

**Cursor / Continue / другие** — у каждого свой способ регистрации
MCP-сервера, команда и аргументы те же.

## Именование интеграций

Имя интеграции — короткая строка вида `[a-z0-9][a-z0-9-]{0,63}`.
Соответствует префиксу файлов в статическом и временном деревьях:

| Имя | Файл конфигурации (статика)              | Файл сессии (по пользователю)                              |
|---|------------------------------------------|------------------------------------------------------------|
| `claude-code` | `/etc/totp-presence/claude-code-config`  | `/var/run/totp-presence/<user>/claude-code-session`        |
| `aider`       | `/etc/totp-presence/aider-config`        | `/var/run/totp-presence/<user>/aider-session`              |

Имена с `..`, `/`, заглавными буквами отклоняются сервером. Ядро
дополнительно проверяет путь на своей стороне — двойная защита от
обхода каталогов.

## Почему sudo без пароля

Сервер запускается под обычным пользователем. При вызове verify
использует `sudo -n` (не спрашивать пароль; если `NOPASSWD` не
настроен — сразу ошибка). Правило sudoers ядра разрешает
беспарольный sudo только для `/etc/totp-presence/verify`.

## Границы

Сервер — тонкая обёртка. Он не читает секретный ключ
(`root:wheel 600`, доступен только ядру), не пишет сессию напрямую
(только ядро, при верном коде), не создаёт интеграции (отсутствие
конфигурационного файла — ошибка) и не прописывает себя в
конфигурацию клиента (добавляется вручную).

## Диагностика проблем

**`sudo: a password is required`** — правило sudoers не установлено.
Проверить: `./core/setup.sh status`.

**`integration 'X' is not installed`** — нет файла конфигурации
`/etc/totp-presence/X-config`. Вызвать `totp_status`, чтобы увидеть
доступные интеграции.

**`fastmcp package not installed`** — `pip3 install fastmcp`. Если
Python через pyenv или conda, убедиться, что MCP-клиент запускает
нужный python — при необходимости указать полный путь в `"command"`.
