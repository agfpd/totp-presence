...
- **Bash** — хук парсит содержимое shell-команды и проверяет
  текстовым поиском наличие путей к конфигурационным файлам.
  Команды записи на такие файлы блокируются: `echo >> settings.json`,
  `sed -i CLAUDE.md`, `cp`, `mv`, `rm`, `tee`, `chmod`, `chown`,
  `find -delete` и подобные. Диагностические чтения тех же файлов
  проходят без проверки TOTP-сессии — `cat ~/.claude/settings.json`,
  `grep hook settings.json`, `head -20 CLAUDE.md`,
  `jq .mcpServers .claude.json` и другие команды, у которых
  первое слово входит в короткий список утилит только для чтения
  (`cat`, `head`, `tail`, `grep`, `wc`, `stat`, `file`, `ls`, `diff`,
  `awk`, `find`, `jq`, `cut`, `sort`, `uniq`, …) и в которых нигде
  не встречаются признаки записи. Неоднозначные команды попадают
  в обычную проверку с укороченным окном — это исправимо
  запросом свежего кода у владельца.
